 ...
 *
 */
public class PickFrontBehavior extends PickMouseBehavior {

   PropertyChangeSupport propertyChangeSupport;
   Bounds bounds;
   BranchGroup root;

   public PickFrontBehavior(BranchGroup root, Canvas3D canvas, Bounds bounds){
   ...
   public void updateScene(int xpos, int ypos){
	   ...
	   if(!mevent.isAltDown() && !mevent.isMetaDown()) {  // ButtonOne
		   ...
		   if(primitive != null && primitive instanceof EnigmaFront) {
			   EnigmaFront panel = (EnigmaFront)primitive;
			   if(propertyChangeSupport != null)
				   propertyChangeSupport.firePropertyChange("EnigmaFrontSelection", null, panel);
		   }
	   }
   }
